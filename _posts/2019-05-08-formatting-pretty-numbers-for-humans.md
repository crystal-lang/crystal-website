...
[*How the world separates its digits*] provides an overview of international styles, and the [Wikipedia article on *Decimal Separators*](https://en.wikipedia.org/wiki/Decimal_separator) provides some more insight.

### Decimal places

Floating point numbers can produce a lot of decimal places when converted to a human-readable string. For user output such detail is usually not intended and displaying a few decimal places is plenty.

Instead of having to round the number manuall, the number of decimal places can be configured directly in the `#format` method:
...
The number of decimal places is fixed by default. Trailing zeros will only be emitted when `only_significant` is `true`:
...
The implementation is not perfect, though. Localization is complex and hard to get right. As always, the devil lies in the details. For example, the thousands delimiter and group size are configurable, but have fixed values. The [Indian numbering system](https://en.wikipedia.org/wiki/Indian_numbering_system) can't be represented in this way. Only arabic numbers are supported. And there are probably lots other cases which would require more special behaviour.

But it's probably good for more than 90% of typical use cases, and already useful in many places. There is always room for improvement.
