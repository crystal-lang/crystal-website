...
This release includes [237 changes since
1.18.2](https://github.com/crystal-lang/crystal/pulls?q=is%3Apr+milestone%3A1.19.0)
...
For example, the value printed by {% raw %}`{{ flag?(:name) }}`{% endraw %} may now be:

- _(not defined)_ => `false`
- `-Dname` => `true`
- `-Dname=value` => `"value"`
...
The monotonic clock has been adjusted on Linux and macOS to include suspended
time, when they previously counted the system uptime.
...
[RFC 0015]: https://github.com/crystal-lang/rfcs/blob/master/text/0015-time-monotonic.md
...
`Process.run` and `Process.exec` now look up the target executable in the
environment of the parent process (`ENV["PATH"]`) instead of in the environment of the
child process (`env["PATH"]`) on all platforms ([#16340]).
This is usually the expected and specified behaviour for spawning a new process,
but it only worked correctly on Windows until now.
Consider using a full path (possibly resolved via [`Process.find_executable`])
if you want to look up the executable in a different path.
With `shell: true`, the target executable is also looked up in `env["PATH"]`
because the shell process sits in between.
...
[`Process.find_executable`]: https://crystal-lang.org/api/1.19.1/Process.html#find_executable%28name%3APath%7CString%2Cpath%3AString%7CNil%3DENV%5B%22PATH%22%5D%3F%2Cpwd%3APath%7CString%3DDir.current%29%3AString%7CNil-class-method
...
[`NamedTuple#reverse_merge`]: https://crystal-lang.org/api/1.19.0/NamedTuple.html#reverse_merge(other:NamedTuple)-instance-method
[`Set#map!`]: https://crystal-lang.org/api/1.19.0/Set.html#map!(&:T-%3ET):self-instance-method
[`Hash#transform_keys!`]: https://crystal-lang.org/api/1.19.0/Hash.html#transform_keys!(&:K,V-%3EK):self-instance-method
[`OpenSSL::SSL::Context::Server#on_server_name`]: https://crystal-lang.org/api/1.19.0/OpenSSL/SSL/Context/Server.html#on_server_name(&block:String-%3EOpenSSL::SSL::Context::Server|Nil)-instance-method
[`IPSocket#ipv6_only`]: https://crystal-lang.org/api/1.19.0/IPSocket.html#ipv6_only?:Bool-instance-method
[`Int.from_digits`]: https://crystal-lang.org/api/1.19.0/Int32.html#from_digits(digits:Enumerable(Int),base:Int=10):self-class-method
[`BigInt.from_digits`]: https://crystal-lang.org/api/1.19.0/BigInt.html#from_digits(digits:Enumerable(Int),base:Int=10):self-class-method
[`Int#digits`]: https://crystal-lang.org/api/1.19.0/Int.html#digits(base=10):Array(Int32)-instance-method
[`Int#tdivmod`]: https://crystal-lang.org/api/1.19.0/Int.html#tdivmod(other:Int)-instance-method
[`Random.next_bool`]: https://crystal-lang.org/api/1.19.0/Random.html#next_bool%3ABool-class-method
[`Random.next_int`]: https://crystal-lang.org/api/1.19.0/Random.html#next_int%3AInt32-class-method
[`Random#split`]: https://crystal-lang.org/api/1.19.0/Random.html#split%3Aself-instance-method
[`Pointer#fill`]: https://crystal-lang.org/api/1.19.0/Pointer.html#fill(count:Int,value:T):self-instance-method
[`Process.debugger_present?`]: https://crystal-lang.org/api/1.19.0/Process.html#debugger_present?:Bool-class-method
[`Path#relative?`]: https://crystal-lang.org/api/1.19.0/Path.html#relative?:Bool-instance-method
[`YAML::Nodes.parse_all`]: https://crystal-lang.org/api/1.19.0/YAML/Nodes.html#parse_all(string_or_io:String%7CIO):Array(Document)-class-method
[`::exit(Process::Status)`]: https://crystal-lang.org/api/1.19.0/toplevel.html#exit(status:Int32|Process::Status=0):NoReturn-class-method
...
The compiler is now always built with execution contexts, and the LLVM codegen
is now always multithreaded for every target, including Windows. We can finally
wave goodbye to fork, and Windows should see a performance boost.
...
We finally build Linux ARM64 binaries of Crystal releases and nightlies.
Tarballs are ready to download from [GitHub releases], [docker images],
[snapcraft] or the [install-crystal] GitHub action.
...
[github releases]: https://github.com/crystal-lang/crystal/releases/tag/1.18.2
[docker images]: https://hub.docker.com/r/crystallang/crystal
[snapcraft]: https://snapcraft.io/crystal
