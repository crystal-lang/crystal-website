...
so nobody has to extend the `libxml2` integration themselves. Follow the
progress in [#15915].
...
The `blocking` parameter used in [`Socket`] constructors, [`IO.pipe`] and
[`IO::Stapled.pipe`] changes from `false` to `nil` by default. Each event loop
...
- On Windows, we don't set the socket to non-blocking anymore. There really is
  no need for that with overlapped IO.
