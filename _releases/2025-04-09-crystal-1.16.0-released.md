...
* The suffixes `?` and `!` are no longer allowed for parameter names of defs,
  macros and blocks. This aligns them with other variable names.
  It only produces a warning for now ([#12197]).

* [`Enumerable#sum`] and [`#product`][`Enumerable#product`] no longer resolve an implicit return type
  when the element type is a union. This can't work reliably.
...
* [`HTTP::Request`] was fixed to correctly parse HTTP resource strings that looks
  like an absolute URL. This may break code that relied on the previous, buggy
  behaviour ([#15499]).

* The compiler won't set the environment variable `$CRYSTAL` when running a
  process for a subcommand. This variable was only in introduced in [#14953] in
  1.14.0. We introduced a more flexible `$CRYSTAL_EXEC_PATH`, and the equivalent
  is now `$CRYSTAL_EXEC_PATH/crystal` ([#15186]).
...
* Several improvements to `Path` and `File` on Windows

### Compiler

### Compiler tools

The doc generator can build documentation for private and protected features as well as lib bindings.
[RFC 0011](https://github.com/crystal-lang/rfcs/blob/main/text/0011-extending-api-docs.md)
