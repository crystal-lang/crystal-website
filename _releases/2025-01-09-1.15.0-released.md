...
`Log` messages do not emit when issues with a block and the block outputs `nil`.
This was incorrectly also the case even when an `exception` instance was passed
to the emit method. This was now fixed in [#15253] and message always emit when
an exception is passed. And we added new overloads for passing an exception
