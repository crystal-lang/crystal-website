...
#### Compiler flags now have values ([#16310])
...
`StringLiteral`. A key without a value returns `true` as implicit value, as
before.
...
#### Adjust monotonic clocks ([#16516])

UNIX targets still use `CLOCK_MONOTONIC` with a couple noteworthy exception:

- Linux now uses `CLOCK_BOOTTIME` to include suspended time;
- macOS now uses `CLOCK_MONOTONIC_RAW` instead of `mach_absolute_time`
  ([#16492]), because `CLOCK_MONOTONIC` only has microsecond resolution.
...
_Thanks, [@andrykonchin], [@carlhoerberg, @HertzDevil, @ysbaddaden, @straight-shoota, @RX14, @Sija, @spuun]
...
[`Proc#[]`]: https://crystal-lang.org/api/master/Proc.html#[](*args:*T):R-instance-method
...
- All overloads of `ArrayLiteral#[]` return `nil` on out of bounds ([#16453], thanks )
