...
Numeric elements are now _autocasted_ to fit into larger types. For instance, it is possible to call a function expecting an `Int64` with an `Int32`:
...
Similarly, unsigned integer types can be casted into larger signed ones, and it also works for `Float32` (to be casted as `Float64`).
